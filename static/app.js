...
const chatUiHelpers = window.HermesMiniappChatUI || {
  getOrCreateTabNode({ tabNodes, tabTemplate, chatId }) {
    const key = Number(chatId);
    if (!Number.isInteger(key) || key <= 0) return null;
    if (tabNodes.has(key)) return tabNodes.get(key);
  ...
  getTabBadgeState({ chat, pendingChats, unseenStreamChats }) {
    const key = Number(chat?.id);
    if (!Number.isInteger(key) || key <= 0) return { text: "", classes: [], ariaLabel: "" };
    const pending = pendingChats.has(key) || Boolean(chat?.pending);
    const unread = Math.max(0, Number(chat?.unread_count || 0));
    const hasUnseen = unseenStreamChats.has(key);
    if (pending) return { text: "…", classes: ["is-visible", "is-pending"], ariaLabel: "Pending response" };
    if (unread > 0 || hasUnseen) {
      return {
        text: "•",
        classes: ["is-visible", "is-unread-dot"],
        ariaLabel: unread > 0 ? `${unread} unread ${unread === 1 ? "message" : "messages"}` : "New messages below current scroll position",
      };
    }
  ...
  applyTabBadgeState({ badge, badgeState }) {
    ...
    if (state.ariaLabel) badge.setAttribute("aria-label", state.ariaLabel);
  },
  applyTabNodeState({ node, chat, activeChatId, pendingChats, unseenStreamChats, getTabBadgeState, applyTabBadgeState }) {
    if (!node || !chat) return;
    ...
    if (pinEl) pinEl.textContent = chat.is_pinned ? "📌" : "";
    ...
    if (badgeEl) applyTabBadgeState(badgeEl, getTabBadgeState(chat));
  },
  ...
  renderTabs({ chats, tabNodes, tabTemplate, tabsEl, applyTabNodeState }) {
    const ordered = [...chats.values()].sort((a, b) => a.id - b.id);
    const nextIds = new Set(ordered.map((chat) => Number(chat.id)));
    this.removeMissingTabNodes({ tabNodes, nextIds });
    ordered.forEach((chat) => {
      const node = this.getOrCreateTabNode({ tabNodes, tabTemplate, chatId: chat.id });
      if (!node) return;
      applyTabNodeState(node, chat);
      if (node.parentElement !== tabsEl) tabsEl.appendChild(node);
    });
  },
  renderPinnedChats({ pinnedChatsWrap, pinnedChatsEl, pinnedChats, doc = document }) {
  ...
  refreshTabNode({ chatId, tabNodes, chats, applyTabNodeState }) {
    const key = Number(chatId);
  ...
  syncActiveTabSelection({ previousChatId, nextChatId, tabNodes, renderTabs, refreshTabNode }) {
...
if (!window.HermesMiniappChatUI) {
  console.warn("HermesMiniappChatUI missing; using inlined fallback helpers.");
}
