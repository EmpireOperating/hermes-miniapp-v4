...
- Create/modify docs: `docs/miniapp-worker-isolation-runbook.md`
...
Files:
- Create/update: `docs/miniapp-child-spawn-hardening-runbook.md`
- Create: `docs/plans/2026-04-02-miniapp-chat-isolation-two-track-plan.md` (this file)
...
### Task B1: Upgrade spawn-lineage logs
- Write failing tests for plain-text spawn/finish logs.
- Add logs to all relevant spawn paths.
- Run tests.
- Commit.

### Task B2: Add fan-out counters/high-water marks
- Write failing tests for per-chat/per-job child counters.
- Expose in runtime status.
- Run tests.
- Commit.

### Task B3: Add transport/fallback transition logs
- Write failing tests for persistent->direct and direct->cli fallback attribution.
- Implement logs.
- Run tests.
- Commit.

### Task B4: Create reproducible storm fixtures/scripts
- Add synthetic tests and a local repro script.
- Verify one scenario actually exercises storm-like behavior.
- Commit.

### Task B5: Patch confirmed trigger
- Only after evidence.
- Add failing regression test from actual root cause.
...
# Suggested file map for the refactor

Likely new files:
- `chat_worker_runner.py`
- `job_runtime_worker_launcher.py`
- `job_runtime_event_relay.py` (if needed)
- `tests/test_chat_worker_runner.py`
...
- `tests/test_routes_jobs_runtime.py`
- `tests/test_routes_chat_stream.py`
...
Minimum targeted commands after each slice:

```bash
source .venv/bin/activate
python -m pytest -q tests/test_hermes_client.py
python -m pytest -q tests/test_job_runtime_chat_job.py
python -m pytest -q tests/test_routes_jobs_runtime.py
python -m pytest -q tests/test_routes_chat_stream.py
```

When worker-launcher files land:

```bash
source .venv/bin/activate
python -m pytest -q tests/test_chat_worker_runner.py tests/test_job_runtime_worker_launcher.py
```

When storm-forensics slices land:
...
Service-level verification after Track A slices:

```bash
systemctl --user restart hermes-miniapp-v4.service
systemctl --user status hermes-miniapp-v4.service --no-pager
curl -sS http://127.0.0.1:8787/health
```

Operator verification goals:
- backend remains up if a worker dies
- one chat can fail locally without global outage
- runtime status shows worker topology and child fan-out diagnostics
- broad repo sweep prompt no longer threatens whole-service OOM

---

# Open design decisions that must be made explicitly

1. Worker isolation mechanism
- plain subprocess only
- subprocess + cgroup wrapper
- transient systemd scope
- separate long-lived worker service pool

Recommendation: prefer an OS-enforced boundary, not a Python-only promise.

2. Persistence ownership
- shared-process persistent runtime is architecturally suspect for true isolation
- likely move persistence to worker-owned state or checkpoint-only continuity

3. Event relay method
- direct parent-child pipe
- DB-backed event writes only
- in-memory broker plus parent relay

Recommendation: preserve DB/event compatibility first; optimize later.

4. Retry ownership
- parent orchestrator should own retries and job state transitions
- worker should execute exactly one claimed turn attempt

---

# Risks and mitigations

Risk: architecture split breaks streaming UX
- Mitigation: preserve current SSE event contract and buffer/resume semantics first; defer optimizations

Risk: worker isolation introduces cold-start latency
- Mitigation: measure it explicitly; prefer correctness/isolation first, warm reuse second

Risk: persistence refactor reintroduces shared-failure coupling
- Mitigation: require explicit design review before keeping any warm state inside the shared backend process

Risk: instrumentation still misses descendant subprocesses
- Mitigation: log pid/ppid/process group and high-water counts; ensure cleanup paths are visible in plain text logs

Risk: broad implementation loses scope
- Mitigation: execute in small slices with tests and commits after each slice
...
1. One chat can delegate freely without capability restriction.
2. One chat’s runaway execution cannot OOM-kill the shared miniapp backend service.
3. Runtime status clearly shows which job/chat is consuming child-process resources.
4. A reproduced storm is attributable to a concrete spawning path.
5. At least one real storm trigger has a regression test and a fix.
6. Chat continuity and streaming remain acceptable to the user.

---

# Immediate next actions for the next agent

If picking this up fresh, do these first in order:

1. Read this plan fully.
2. Read `docs/miniapp-child-spawn-hardening-runbook.md`.
3. Read current execution-path files:
   - `server.py`
   - `job_runtime.py`
   - `job_runtime_chat_job.py`
   - `hermes_client.py`
   ...
   - `hermes_client_agent_direct.py`
   - `hermes_client_cli.py`
4. Verify Track B observability slices (B1/B2/B3/B4) are green and reproducible:
   - `python -m pytest -q tests/test_fanout_storm_forensics.py`
   - `python scripts/repro_fanout_storm.py --scenario all`
5. Begin Track A1 with an isolated chat-runner entrypoint and worker launcher abstraction.
6. Keep delegation enabled; do not solve this by reducing miniapp capabilities.

---

# Handoff note

This is not a “reduce memory” cleanup plan. This is a fault-boundary correction plan.

We are changing the system from:
- many chats sharing one apartment

toward:
- the web app as the building front desk
- isolated worker apartments/work bays for active chat execution
- instrumentation that tells us exactly which apartment caused a surge and why
